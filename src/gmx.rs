...
use ethers::{abi::Abi, prelude::Lazy, types::{transaction::eip2930::AccessList, Eip1559TransactionRequest, NameOrAddress}, utils::keccak256};
use serde::{Deserialize, Serialize};
use ethers::types::{Address, Block, BlockNumber, H160, H256, U256, Bytes};
use ethers_providers::Middleware;

use crate::{interfaces::gmx::{get_position_key, CreateOrderParams, CreateOrderParamsAddresses, CreateOrderParamsNumbers, GmxV2ABI, MarketPrices, PriceProps, Token}, simulator::{EvmSimulator, Tx}};
...
pub static REFERRAL_STORAGE: Lazy<H160> = Lazy::new(|| H160::from_str("0xe6fab3f0c7199b0d34d7fbe83394fc0e0d06e99d").unwrap());
pub static ORDER_VAULT: Lazy<H160> = Lazy::new(|| H160::from_str("0x31eF83a530Fde1B38EE9A18093A333D8Bbbc40D5").unwrap());
pub static READER: Lazy<H160> = Lazy::new(|| H160::from_str("0x2b43c90D1B727cEe1Df34925bcd5Ace52Ec37694").unwrap());
...
impl<M: Middleware + 'static + std::clone::Clone> GmxPlayground<M> {
    ...
    pub async fn create_order(&self, collateral_token: H160,collateral_amount: U256,size_delta_usd: f64, long: bool) -> Bytes {
    ...
    pub async fn create_short_position(&mut self, collateral_token: &str, collateral_amount: f64, size_delta_usd: f64) -> Result<bool> {
        let collateral_token_addr = H160::from_str(&Token::from_name(collateral_token).unwrap().info().address).unwrap();
        ...
        let create_order = self.create_order(collateral_token_addr, collateral_amount, size_delta_usd, false).await;
    ...
    pub fn get_account_positions(&mut self) {
        ...
        let query_call = self.reader.encode("getAccountPositions", (data_store, owner, start_index, end_index)).unwrap();
        let tx = Tx {
            caller: self.simulator.owner,
            transact_to: *READER,
            data: query_call.0,
    ...
    pub fn get_position_info(&mut self, market_token: H160)  {
        ...
        let position_key = get_position_key(self.simulator.owner, market_token, collateral_token, false);
        ...
        let query_call = self.reader.encode("getPositionInfo", (data_store, referral_storage, position_key, market_prices, size_delta_usd, ui_fee_receiver, use_position_size_as_size_delta_usd)).unwrap();
        let tx = Tx {
            caller: self.simulator.owner,
            transact_to: *READER,
            data: query_call.0,
